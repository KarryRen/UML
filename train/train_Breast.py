...
def SOTA_EXPERIMENT_UML_BREAST():
    """ Final Network UML """
    ...
    train_loader = torch.utils.data.DataLoader(
        JointBreastDataset(path_breast_data=config.BREAST_DATA_FILE,
                           ...
                           data_type='train'),
        batch_size=config.BREAST_BATCH_SIZE, num_workers=0, shuffle=True, pin_memory=False, drop_last=True)
    ...
    for epoch in range(config.EPOCH):
        ...
        for i_iter, batch in enumerate(train_loader):
            step = (config.BREAST_TRAIN_NUM / config.BREAST_BATCH_SIZE) * epoch + i_iter
            images, masks, labels, image_name = batch
            images = images.cuda()
            labels = labels.cuda()
            masks = masks.unsqueeze(1).cuda()
            optimizer.zero_grad()
            lr = adjust_learning_rate(optimizer, step, epoch, whole_steps=config.BREAST_STEPS,
                                      base_steps=config.BREAST_BASE_STEPS_OF_30_EPOCHS)
        ...
        if epoch >= config.VALID_EPOCH:
            # ---- save model ---- #
            filename = str(epoch) + '_uml_breast.pt'
            torch.save(model.module.state_dict(), os.path.join(config.MODEL_SAVE_PATH, filename))

            # ---- eval train loader ----#
            train_test_result = eval_UML_BREAST(valid_data_loader=train_test_loader,
                                                model=model,
                                                epoch=epoch,
            ...
            valid_result = eval_UML_BREAST(valid_data_loader=valid_loader,
                                           model=model,
                                           epoch=epoch,
