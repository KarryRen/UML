...
""" The UML_Net training code of ISPY Dataset. """
...
train_loader = data.DataLoader(JointIspyDataset(root_path=config.ISPY_ROOT_PATH, data_type="Train"),
                               batch_size=config.BATCH_SIZE, shuffle=True)  # train dataloader
valid_loader = data.DataLoader(JointIspyDataset(root_path=config.ISPY_ROOT_PATH, data_type="Valid"),
                               batch_size=1, shuffle=False)  # valid dataloader
train_t_loader = data.DataLoader(JointIspyDataset(root_path=config.ISPY_ROOT_PATH, data_type="Train"),
...
for epoch in range(config.EPOCH):
    ...
    for i_iter, batch_data in enumerate(train_loader):
        ...
        cls_labels = batch_data["cls_label"].to(dtype=torch.int64).cuda()  # cls_labels, shape=(bs)
    ...
    if epoch >= config.VALID_EPOCH:
        # 1. save model
        torch.save(model.state_dict(), f"{config.PATH_MODEL}/{epoch}_uml_ispy")
