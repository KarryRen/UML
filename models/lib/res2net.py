...
import torch
from models.lib.convs import FrozenBatchNorm2d
import config as config

# ----- Description ---- #
# This is the class of res2net backbone which is used in classification feature extractor.
# I use it just like a black box.
...
class Res2Net(nn.Module):
    def __init__(self, block, layers, baseWidth=26, scale=4, input_features=False):
        ...
        self.layer1 = self._make_layer(block, 64, layers[0])  # 3 blocks [原本参数是 64]
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2)  # 4 blocks [原本参数是 128]
        self.layer3 = self._make_layer(block, 256, layers[2], stride=2)  # 6 blocks [原本参数是 256]
        self.layer4 = self._make_layer(block, 512, layers[3], stride=2)  # 3 blocks [目前就没再使用了]
...
def res2net101_v1b(pretrained=True, **kwargs):
    ...
    if True:
        ...
        pretrained = torch.load(open("./model_zoo/res2net_cls.pth", "rb"), map_location=torch.device('cpu'))
...
if __name__ == '__main__':
    ...
    model = res2net50_v1b_26w_4s(pretrained=config.PRETRAIN_RES2NET)
    model = model
    for i in range(5):
        print(model(images)[i].shape)
